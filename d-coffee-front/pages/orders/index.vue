...
<template>
  <view class="orders-page">
    ...
    <view v-else-if="errorMessage" class="state-card"><text>{{ errorMessage }}</text><text v-if="!uni.getStorageSync('dcoffee-user-token')" class="action-link" @tap="uni.navigateTo({ url: '/pages/account/index' })">去登录</text><text v-else class="action-link" @tap="loadOrders">重新加载</text></view>
    <view v-else-if="!orders.length" class="state-card"><text class="empty-title">还没有订单</text><text>选好咖啡后，在购物车提交即可创建订单。</text></view>
...
.orders-page { min-height: 100vh; box-sizing: border-box; padding: 28rpx 30rpx 60rpx; background: #f7f3ed; }
...
.order-card { padding: 24rpx; border-radius: 17rpx; background: #fffdfa; }
...
.cancel-button { display: block; width: fit-content; margin: 18rpx 0 0 auto; padding: 11rpx 25rpx; border: 1rpx solid #e5dbd0; border-radius: 30rpx; color: #806449; font-size: 18rpx; }
.cancel-button--busy { opacity: .55; }
.state-card { display: flex; min-height: 55vh; flex-direction: column; align-items: center; justify-content: center; gap: 17rpx; color: #918477; font-size: 21rpx; text-align: center; }
.empty-title { color: #48362a; font-size: 27rpx; font-weight: 600; }
.action-link { padding: 13rpx 28rpx; border-radius: 28rpx; background: #513827; color: #fffaf4; font-size: 20rpx; }
</style>
